...
public class ROSBridgeClient extends ROSClient {
    ...
    public void typeMatch(TypeDef t, Class<? extends Message> c) throws InterruptedException {
        ...
        // Error can trigger because classFieldName and typeFieldName are not in the same order

        List<String> fieldsSorted = new ArrayList<>();
        String value;

        for(Field field : fields) {                           // Iterate over the object's fields

            field.setAccessible(true);                        // Ignore the private modifier
            value = field.getName();                      // Get the value stored in the field
            fieldsSorted.add(value);                  // Add the field name to the list
        }
        ...
        for (int i = 0; i < t.fieldnames.length; i++) {
            ...
            String classFieldName = fieldsSorted.get(i);
            String typeFieldName = t.fieldnames[i];

            //System.out.println("classFieldName: " + classFieldName);
            //System.out.println("typeFieldName: " + typeFieldName);
